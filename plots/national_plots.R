...
# Pull telco data
dat_counts <- read_csv(paste0("outputs/data_", TIMESTAMP, "/national_data.csv")) %>%
  filter(segment %in% c("domestic", "international")) %>%
  mutate(timestamp = as.Date(timestamp))

dat_mobility <- read_csv(paste0("outputs/data_", TIMESTAMP, "/mobility_national_data.csv")) %>%
...
dom_counts <- dat_counts %>% filter(segment=="domestic") %>% 
  ...
  geom_vline(data=past_levels, aes(xintercept = past_levels), colour = dvblack, size=2/2.13) + # The vertical lines....
  geom_vline(xintercept = current_level, colour = "#F0C47F", size=2/2.13) +
...
int_counts <-dat_counts %>% filter(segment=="international") %>% 
  ...
  theme_void() + ## theme_X has some default themes and void is the one that strips everything
  geom_vline(data=past_levels, aes(xintercept = past_levels), colour = dvblack, size=2/2.13) + # The vertical lines....
  geom_vline(xintercept = current_level, colour = "#1EAE9B", size=2/2.13) +
...
mobility <- ggplot(dat_mobility,aes(x=days, y=mobility)) +
  ...
  geom_vline(data=past_levels, aes(xintercept = past_levels), colour = dvblack, size=2/2.13) + # The vertical lines....
  geom_vline(xintercept = current_level, colour = mobility_palette[1], size=2/2.13) +
